function Add-WizardListBox {
    <#
    .SYNOPSIS
    Adds a ListBox selection control to a wizard step.
    ...
    .EXAMPLE
    Add-WizardListBox -Step "Config" -Name "Features" -Label "Select Features:" -Choices @('Web Server', 'Database', 'Cache', 'Monitoring') -MultiSelect
    
    Adds a multi-select ListBox for feature selection.
    
    .EXAMPLE
    Add-WizardListBox -Step "Config" -Name "Priority" -Label "Priority Level:" -Choices @('Low', 'Medium', 'High', 'Critical') -Default 'Medium' -Mandatory
    
    Adds a required single-select ListBox with a default selection.
    
    .EXAMPLE
    Add-WizardListBox -Step "Config" -Name "Regions" -Label "Deployment Regions:" -Choices @('US-East', 'US-West', 'EU-Central', 'APAC') -MultiSelect -Default @('US-East', 'US-West') -Height 200
    
    Adds a multi-select ListBox with multiple defaults and custom height.
    
    .EXAMPLE
    Add-WizardListBox -Step "Config" -Name "Features" -Label "Select Features:" -ScriptBlock {
        param($Environment)
    ...
    .OUTPUTS
    WizardControl object representing the created ListBox.
    ...
    process {
        try {
            ...
            # Create the control
            $control = [WizardControl]::new($Name, $Label, 'ListBox')
            ...
            if ($ScriptBlock) {
                ...
                $control.SetProperty('DataSourceScriptBlock', $ScriptBlock.ToString())
        ...
        Write-Verbose "Add-WizardListBox completed for: $Name"
    }
}
