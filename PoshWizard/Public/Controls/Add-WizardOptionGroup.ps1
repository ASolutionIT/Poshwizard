function Add-WizardOptionGroup {
    <#
    .SYNOPSIS
    Adds an option group (radio button set) to a wizard step.
    ...
    .EXAMPLE
    Add-WizardOptionGroup -Step "Config" -Name "Environment" -Label "Target Environment" -Options @('Dev','Test','Prod') -Default 'Test'

    Adds a horizontal radio button group for environment selection.

    .OUTPUTS
    WizardControl object representing the created option group.
    ...
    process {
        try {
            ...
            $control = [WizardControl]::new($Name, $Label, 'OptionGroup')
        ...
        Write-Verbose "Add-WizardOptionGroup completed for: $Name"
    }
}
