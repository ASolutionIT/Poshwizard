function Add-WizardTextBox {
    <#
    .SYNOPSIS
    Adds a text input control to a wizard step.
    ...
    .EXAMPLE
    Add-WizardTextBox -Step "Config" -Name "Description" -Label "Description:" -Multiline -MaxLength 500
    
    Adds a multi-line text area with character limit.
    
    .EXAMPLE
    Add-WizardTextBox -Step "Config" -Name "Email" -Label "Email Address:" -ValidationPattern "^[^@]+@[^@]+\.[^@]+$" -ValidationMessage "Please enter a valid email address"
    
    Adds a text input with email validation.
    
    .OUTPUTS
    WizardControl object representing the created text input.
    ...
    process {
        try {
            ...
            # Create the control
            $control = [WizardControl]::new($Name, $Label, 'TextBox')
        ...
        Write-Verbose "Add-WizardTextBox completed for: $Name"
    }
}
