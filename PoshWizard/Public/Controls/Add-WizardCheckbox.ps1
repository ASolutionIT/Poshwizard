function Add-WizardCheckbox {
    <#
    .SYNOPSIS
    Adds a checkbox control to a wizard step.
    ...
    .EXAMPLE
    Add-WizardCheckbox -Step "Config" -Name "EnableLogging" -Label "Enable detailed logging"
    
    Adds a basic checkbox for enabling logging.
    
    .EXAMPLE
    Add-WizardCheckbox -Step "Config" -Name "AcceptTerms" -Label "I accept the terms and conditions" -Mandatory
    
    Adds a required checkbox that must be checked to proceed.
    
    .EXAMPLE
    Add-WizardCheckbox -Step "Config" -Name "CreateBackup" -Label "Create backup before installation" -Default $true
    
    Adds a checkbox that is checked by default.
    
    .OUTPUTS
    WizardControl object representing the created checkbox.
    ...
    process {
        try {
            ...
            # Create the control
            $control = [WizardControl]::new($Name, $Label, 'Checkbox')
        ...
        Write-Verbose "Add-WizardCheckbox completed for: $Name"
    }
}
