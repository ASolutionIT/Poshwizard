...
namespace Launcher.Attributes
{
    ...
    public class WizardDataSourceAttribute : Attribute
    {
        /// <summary>
        ...
        public WizardDataSourceAttribute()
        {
        ...
        public WizardDataSourceAttribute(ScriptBlock scriptBlock)
        {
        ...
        public WizardDataSourceAttribute(string csvPath, string csvColumn)
        {
            CsvPath = csvPath;
            CsvColumn = csvColumn;
            DependsOn = new string[0];
        ...
        public void Validate()
        {
            ...
            if (!hasScriptBlock && !hasCsv)
            {
                throw new InvalidOperationException(
                    "WizardDataSource must specify either a ScriptBlock or CSV configuration (CsvPath and CsvColumn).");
            }

            if (hasScriptBlock && hasCsv)
            {
                throw new InvalidOperationException(
                    "WizardDataSource cannot have both ScriptBlock and CSV configuration. Choose one.");
            }
